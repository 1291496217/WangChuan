...
* - 提供交互范围
* - 保存 NPC 名称、当前故事阶段、当前行为状态和 Blueprint 可配置的阶段对话。
...
public:
	...
	*/
	...
	/*
	* 返回当前 Story Stage 对应的稳定 Anchor ID。
	*
	* 当前 Demo 约定：
	* StoryAnchors 下标与 Story Stage 对应。
	*
	* Stage 0 → StoryAnchors[0]
	* Stage 1 → StoryAnchors[1]
	* Stage 2 → StoryAnchors[2]
	*/
...
protected:
	...
	* 从而让最终模型视觉朝向与 Story Anchor 箭头一致。
	*/
